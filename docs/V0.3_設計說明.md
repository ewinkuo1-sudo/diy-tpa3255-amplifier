...
2026-09-11。依使用者要求加入 XLR，保留 RCA，由設計端選擇電感、依 Z10 前級匹配輸入，加入 Trigger 自動開關機，並採用前次音質改善建議。此版為原理圖草案，已有[PCB 初步配置](../electrical/pcb-draft/README.md)，尚未走線或實機驗證。

開始 PCB 封裝核對時，已將 D303 BAT54 從邏輯兩腳符號修正為實體 SOT-23：1=A 接 AUDIO_MR、2=NC、3=K 接 AUX_GOOD；功能方向不變，避免套入封裝時誤接到 NC。依 [BAT54 原廠腳序](https://assets.nexperia.com/documents/data-sheet/BAT54.pdf)，並同步更新接線檢查。
...
整機依使用者要求改為[機內電源、單一市電線](../electrical/system-power/README.md)，48V／12V 均由機內模組供應，J201／J202 為機內接點。Trigger 關機是進入待機：切斷板上功率級的 48V 供電；兩組 AC/DC 及 12V 待機／類比支路仍有電，總耗電待實測。後方總開關處理整機供電，音訊 PCB 不切換市電。交流保護與配線不在本版低壓原理圖內。

...
V0.3 已加入 **XLR 平衡輸入並保留 RCA**，搭配濾波後回授（PFFB）、四顆 Coilcraft MA5172-AE，以及隔離 Trigger、自動啟停與 48V 功率電源開關。原八頁的全部元件已按訊號路徑重新排列成 **一張連續接線的完整電路圖**，可由輸入一路看到放大、喇叭、四路回授及啟停控制；元件與電氣連接維持原版。已開始 PCB 零件配置；以 2×50W / 8Ω 為第一階段目標，尚無完成的板子或實測。
...
總圖採 **A0 橫式單頁**，180 個元件重新編排；音訊由左往右，四路 PFFB 實際接線繞回輸入，下方整合供電與啟停控制。483 個腳位、120 個網路與原版一致，另以移除標籤的接線表確認主要路徑確實連通。PDF／SVG 可放大看數值。建議直接把單頁 PDF 傳給教授，螢幕閱讀時放大；列印以 A0 原尺寸為佳，縮到 A4 文字會太小。供電、偏壓與部分狀態線使用同名標籤相連；導線交叉處有圓點才相接。

...
使用者補充希望聲音更好；已加入 `docs/V0.2_音質改善評估.md`，本次僅設計審查，未修改 V0.2 原理圖。PFFB 與 XLR 是待評估方案，不能視為已實作或已驗證。

1. 先評估 PFFB 與 LC／頻響，再確定輸入增益與電感（含偏磁後電感、過流門檻與熱）及輸出電容。原廠 PFFB 範例會降低功率級增益，且需驗證空載等條件的穩定性；現有區塊模型不足以驗證整個迴路。
